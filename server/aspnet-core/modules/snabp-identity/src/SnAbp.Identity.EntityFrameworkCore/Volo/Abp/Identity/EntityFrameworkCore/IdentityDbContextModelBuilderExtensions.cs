    ...
    public static class IdentityDbContextModelBuilderExtensions
    {
            ...
            [CanBeNull] Action<IdentityModelBuilderConfigurationOptions> optionsAction = null)
        {
            ...
            var options = new IdentityModelBuilderConfigurationOptions(
                ...
                SnAbpIdentityDbProperties.DbSchema
            );
            ...
            builder.Entity<IdentityUser>(b =>
            {
                ...
                b.Property(u => u.NormalizedEmail).IsRequired(false)
                ...
                b.HasMany(u => u.Organizations).WithOne().HasForeignKey(ur => ur.UserId).IsRequired();
            ...
            builder.Entity<IdentityUserRltRole>(b =>
            {
            ...
            {
                b.ToTable(options.TablePrefix + "Organization", options.Schema);

                b.ConfigureByConvention();

                b.Property(ou => ou.Code).IsRequired().HasMaxLength(OrganizationConsts.MaxCodeLength)
                    .HasColumnName(nameof(Organization.Code));
                b.Property(ou => ou.Name).IsRequired().HasMaxLength(OrganizationConsts.MaxDisplayNameLength)
                    .HasColumnName(nameof(Organization.Name));

               // b.HasMany<Organization>().WithOne().HasForeignKey(ou => ou.ParentId);

                b.HasMany(ou => ou.Roles).WithOne().HasForeignKey(our => our.OrganizationId).IsRequired();

                b.HasIndex(ou => ou.Code);
            });

            builder.Entity<OrganizationRltRole>(b =>
            {
                b.ToTable(options.TablePrefix + "OrganizationRltRoles", options.Schema);

                b.ConfigureByConvention();

                b.HasKey(ou => new {ou.OrganizationId, ou.RoleId});

                b.HasOne<IdentityRole>().WithMany().HasForeignKey(ou => ou.RoleId).IsRequired();

                b.HasIndex(ou => new {ou.RoleId, ou.OrganizationId});
            });

            builder.Entity<IdentityUserRltOrganization>(b =>
            {
                b.ToTable(options.TablePrefix + "UserRltOrganization", options.Schema);

                b.ConfigureByConvention();

                b.HasKey(ou => new {ou.OrganizationId, ou.UserId});

                b.HasOne<Organization>().WithMany().HasForeignKey(ou => ou.OrganizationId).IsRequired();

                b.HasIndex(ou => new {ou.UserId, ou.OrganizationId});
            });
            ...
            builder.Entity<IdentityUserRltProject>(b =>
            {
                b.ToTable(options.TablePrefix + "IdentityUserRltProject", options.Schema);
                b.ConfigureByConvention();
            });
        }
    }
