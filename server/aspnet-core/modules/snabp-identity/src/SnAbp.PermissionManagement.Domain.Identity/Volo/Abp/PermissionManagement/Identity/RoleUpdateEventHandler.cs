...
namespace SnAbp.PermissionManagement.Identity
{
        ...
        ITransientDependency
    {
        ...
        public virtual async Task HandleEventAsync(IdentityRoleNameChangedEvent eventData)
        {
            ...
            var permissionGrantsInRole = await PermissionGrantRepository.GetListAsync(RolePermissionValueProvider.ProviderName, eventData.OldGuid);
            foreach (var permissionGrant in permissionGrantsInRole)
            {
                await PermissionManager.UpdateProviderGuidAsync(permissionGrant, eventData.OldGuid);
            }
        }
    }
}