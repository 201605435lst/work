...
export default {
  name: 'SmProjectDossierCatrgotyTreeSelect',
  model: {
    prop: 'value',
    event: 'change',
  },
  ...
  data() {
    return {
      dossierCategories: [], // 列表数据源
      dossierCategoriesFlat: [], //平状数据源
      iValue: null,
  ...
  methods: {
    ...
    isValueLoading() {
      ...
      if (this.value instanceof Array) {
        if (this.value.length > 0) {
          // 保证数组里面的所有数据已经加载
          if (this.value.some(id => this.dossierCategoriesFlat.find(x => x.id == id) == null)) {
            refresh = true;
          }
        } else {
          if (this.value.length == 0 && this.dossierCategoriesFlat.length == 0) {
            refresh = true;
          }
        }
      }
      // 当是单选的时候
        ...
        if (this.value) {
          if (this.dossierCategoriesFlat.find(x => x.id === this.value) == null) {
            refresh = true;
          }
        } else {
          if (!this.value && this.dossierCategoriesFlat.length == 0) {
            refresh = true;
          }
        }
      }
    ...
    async refresh(keyWords, isReset) {
      ...
      //是否刷新
      if (isValueLoading) {
        let response = await apiDossierCategory.getList({
          parentId: null,
        ...
        if (requestIsSuccess(response) && response.data) {
          this.dossierCategories = [];
          this.dossierCategoriesFlat = [];
          let _dossierCategories = treeArrayItemAddProps(response.data, 'children', [
            {
              targetProp: 'title',
              handler: item => {
          ...
          this.dossierCategoriesFlat = treeArrayToFlatArray(_dossierCategories);

          this.dossierCategories = _dossierCategories;
        }
      }
    },
    ...
    async onLoadData(treeNode) {
      if (treeNode.dataRef.children && treeNode.dataRef.children.length == 0) {
        let response = await apiDossierCategory.getList({ parentId: treeNode.dataRef.value, ids: [], isAll: true });
        if (requestIsSuccess(response) && response.data) {
          this.dossierCategoriesFlat = this.dossierCategoriesFlat.concat(response.data);
    ...
    async setValue() {
      let result = await this.processData(this.dossierCategoriesFlat, this.value);
      if (result) {
        if (this.treeCheckable) {
  ...
  render() {
    return (
      <a-tree-select
        ...
        treeData={this.dossierCategories}
        value={this.iValue}
