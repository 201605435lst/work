...
export default {
  ...
  props: {
    ...
    isImportButton: { type: Boolean, default: true }, //控制图标
    repairTagKey: { type: String, default: null }, //维修项标签
    icon:{type:String,default:'import'},
    uploadInfo: { type: File, default: null }, //文件呢导入模板选择框的数据
  },
  data() {
    return {
      progress: 0, // 进度
  ...
  methods: {
    ...
    uploadInfoAction() {
      this.file = this.uploadInfo;
      // 开始改变状态
      this.canImport = this.file ? false : true;
      this.isComplete = this.file ? false : true;
      this.fileName = this.file ? this.file.name : '';
      this.fileType = this.file ? this.file.type : '';
      this.$emit('upLoadFile');
      this.file ? this.getProgress() : this.$emit('success', 'error');
    },
    // 暂时保留
    execteImport() {
      // 调用接口，执行导入
    ...
    async exect(data) {
      ...
      if (this.repairTagKey != null) formData.repairTagKey = this.repairTagKey;
    ...
    return (
      <div class="sm-import">
        <a-button
          ...
          type={this.btnType}
          icon={this.isImportButton ? 'import' : null}
          onClick={() => {
            this.check();
            if (this.canImport && this.isImportButton) {
              this.$refs.fileInput.click();
            } else {
              this.uploadInfoAction();
            }
          }}
        >
