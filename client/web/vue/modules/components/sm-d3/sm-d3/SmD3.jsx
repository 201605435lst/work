...
export default {
  ...
  data() {
    return {
      ...
      d3SafePanelPosition: { left: '20px', bottom: '20px' },
      d3QualityPanelPosition: { left: '20px', bottom: '20px' },
  ...
  methods: {
    ...
    async onFlyTo(data, selected = false) {
      if (d3.snDynamicRectLabelHelper) {
        d3.snDynamicRectLabelHelper.clear();
      }
      ...
      this.selectedModels.map(item => {
        this.setModelState(`${item.groupName}@${item.name}`, ModelState.IsSelected, false, false);
      });
    ...
    onInterfaceFlyTo(data) {
      let { groupName, name, state, position } = data;
      let { lon, lat, alt, heading, pitch, roll } = JSON.parse(position);
      ...
      d3.scene.camera.flyTo({
        destination: JSON.parse(position).position,
        orientation: {
          heading,
          pitch,
          roll,
    ...
    async initScene() {
      ...
      d3.snClippingBox = new SnCesiumX.SnClippingBox(d3.viewer);
    ...
    installEvents() {
      ...
      spaceEventHandlerEvent = click => {
        if (d3Manager.mode === D3Mode.Select || d3Manager.mode === D3Mode.SetCamera) {
          ...
          if (feature && d3.snClippingBox.isOpened()) {
            d3.snClippingBox.close();
            d3.snClippingBox.open();
            d3.snClippingBox.setTarget(feature.tileset);
          }
    ...
    onScopeCodeChange(value) {
      ...
      if (this.select && this.selectedEquipments) {
      ...
      <div class="sm-d3">
        ...
        {/* 工具栏 */}
        {
          <ToolBar
            ...
            smD3QualityPanelVisible={this.smD3QualityPanelVisible}
            smD3SafePanelVisible={this.smD3SafePanelVisible}
        ...
        <div class="panel-layer" ref="panelLayer">
            ...
            position={{
              ...
              bottom:
                this.teminalLinkPanelVisible ||
                  ...
                  this.smD3QualityPanelVisible ||
                  this.smD3SafePanelVisible ||
          ...
          {!this.select ?
            <SmD3Interface
              ...
              onFlyTo={data => {
                let { groupName, name, state, position } = data;
                let _position = JSON.parse(position);
                if (_position instanceof Object) {
                  this.onInterfaceFlyTo(data);
                } else {
                  this.onFlyTo(data, true);
                }
          ...
          {!this.select ?
            <SmD3Safe
              ...
              height="320px"
              width="840px"
              position={this.d3SafePanelPosition}
          ...
          {!this.select ?
            <SmD3Quality
              ...
              height="320px"
              width="840px"
              position={this.d3QualityPanelPosition}
