...
export default {
  ...
  props: {
    ...
    isShowStation:{type:Boolean,default:false},//是否只显示站点
  },
  data() {
    return {
      stationOrgs: [],
      ...
      iOrganization: null,
      iRailwayId: null,
      iStaRelateType: RelateRailwayType.SINGLELINK,
    };
  },
  computed: {},
  watch: {
  ...
  methods: {
    ...
    async refresh() {
      let response = await apiStation.getCascaderList(
        {
          organizationId: this.iOrganization,
          railwayId: this.iRailwayId,
          isShowUpAndDown: this.isShowUpAndDown,
          isShowStation: this.isShowStation,
        });
    ...
    findItemInChildren(targetId, data) {
      for (let item of data.children) {
        ...
        let relateType = 11;
        if (this.iStaRelateType != RelateRailwayType.SINGLELINK) relateType = this.iStaRelateType;
          ...
          if (resId != null) {
            for (let i = 0; i < this.listData.length; i++) {
              ...
              if (item.id == resId && (item.type == 1 || item.type == 2 || item.type == 3 || item.type == 11)) {
                ...
                relateId = item.id;
                isQualify = true;
                this.iStaRelateType = selectedStaRelateType;
                break;
              }
            }
          }
