...
export default {
  ...
  methods: {
    ...
    async getYearMonthPlanStatisticsData() {
      ...
      let response = await yearMonthPlanStatistics.getYearPlanProgress(
        this.queryParams,
        this.repairTagKey,
      );
      if (requestIsSuccess(response)) {
        ...
        if (response.data[0] != undefined) {
          this.equipmentOrganizationId = response.data[0].orgId;
        }
        this.resonseData = response.data;
        //构造年月表完成情况柱状图数据
        let totalCount = 0;
        let finishCount = 0;
        let alterCount = 0;
        ...
        response.data.map((i, index) => {
          ...
          this.yearMonthPlanUnFinshPercentData.push(
            this.percentFilter(subNum(100, addNum(finsihP, alterP))),
          );
          ...
          unFinishCount = totalCount - finishCount - alterCount;
    ...
    renderYearMonthPlanChart() {
      ...
      let title =
        this.queryParams.type == DateReportType.Year
          ? preTitle + '年表完成进度'
          : this.queryParams.type == DateReportType.Month
            ? preTitle + ' 月表完成进度'
            : '';
      // 年月表完成统计图
      ...
      yearMonthPlanFinishPercent.on('click', params => {
        this.equipmentOrganizationId = this.resonseData.find(x => x.orgName === params.name).orgId;
        //渲染右侧 底部 各个设备完成情况统计图
        this.renderEveryEquipFinishChart();
      });
    },
    ...
    renderEveryEquipFinishChart() {
      ...
      this.repairGroupNames = [];
      console.log(this.allRepairGroupFinishPercentData);
      let data = this.allRepairGroupFinishPercentData.find(
        s => s.orgId == this.equipmentOrganizationId,
      );
      console.log(data);
      let finishData = [];
      let alterData = [];
      ...
      if (data == null || data == undefined || data.repairGroupFinishData.length == 0) {
        ...
        data.repairGroupFinishData.map((i, index) => {
          ...
          unFinishData.push(Math.abs(this.percentFilter((100 - finsihP - alterP).toFixed(2))));
        });
        ...
        equipmentChart.on('click', async function(params) {
          ...
          let response = await yearMonthPlanStatistics.getRepairGroupFinishData(
            param,
            page.repairTagKey,
          );
          if (requestIsSuccess(response)) {
    ...
    renderSingleEquipFinishInfo(name, data) {
      ...
      data.map((item, index) => {
        ...
        finishData.push(finsihP);
        alterData.push(alterP);
        let unfinsihP = this.percentFilter((100 - finsihP - alterP).toFixed(2));
        if (unfinsihP <= 0) {
          unfinsihP = 0;
        }
        unFinishData.push(unfinsihP);
      });
      echarts.dispose(this.$refs.equipmentChart);
