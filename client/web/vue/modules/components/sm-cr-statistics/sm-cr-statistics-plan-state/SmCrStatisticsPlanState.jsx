...
import ApiYearMonthPlanStatistical from '../../sm-api/sm-cr-plan/YearMonthPlanStatistical';
import FileSaver from 'file-saver';

let yearMonthPlanStatistical = new ApiYearMonthPlanStatistical();
...
export default {
  ...
  methods: {
    initAxios() {
      ...
      yearMonthPlanStatistical = new ApiYearMonthPlanStatistical(this.axios);
    },
    ...
    async exportPlanList() {
      let state = this.queryParams.state === undefined ? 2 : this.queryParams.state;
      let repairType = this.queryParams.repairType == undefined ? 3 : this.queryParams.repairType;//RepairType.Muster,
      let data = {
        organizationId: this.queryParams.organizationId,
        monthPlanType: this.queryParams.planType,
        planTime: this.queryParams.selectedMonthValue,
        state: state,
        repairTagKey: this.repairTagKey,
        repairlType: repairType,
      };
      this.loading = true;
      let response = null;
      switch (this.activeTabKey) {
      case '0':
        response = await yearMonthPlanStatistical.exportMonthCompletion(data);
        if (requestIsSuccess(response) && response.data) {
          FileSaver.saveAs(
            new Blob([response.data], { type: 'application/vnd.ms-excel' }),
            `${moment(data.planTime).year()}年${moment(data.planTime).month() + 1}月-${
              this.orgName
            }-${getDateReportTypeTitle(data.monthPlanType)}-月完成情况统计表.xlsx`,
          );
        }
        this.loading = false;
        break;
      case '1':
        response = await yearMonthPlanStatistical.exportMonthCompletionRates(data);
        if (requestIsSuccess(response) && response.data) {
          FileSaver.saveAs(
            new Blob([response.data], { type: 'application/vnd.ms-excel' }),
            `${moment(data.planTime).year()}年${moment(data.planTime).month() + 1}月-${
              this.orgName
            }-${getDateReportTypeTitle(data.monthPlanType)}-月完成率情况统计表.xlsx`,
          );  
        } 
        this.loading = false;
        break;
      case '2':
        response = await yearMonthPlanStatistical.exportYearCompletionRates(data);
        if (requestIsSuccess(response) && response.data) {
          FileSaver.saveAs(
            new Blob([response.data], { type: 'application/vnd.ms-excel' }),
            `${moment(data.planTime).year()}年${moment(data.planTime).month()+1}月-${
              this.orgName}年表完成率情况统计表.xlsx`,
          );
        }
        this.loading = false;
        break;
  ...
  render() {
    let button = (
      <div style="float:right;padding-right: 40px;">
        ...
        <a-button
          onClick={this.exportPlanList}
          style="margin-left: 10px;background-color:#4addf5c7"
