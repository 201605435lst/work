...
export default {
  name: 'SmStdBasicIndividualProjectTreeSelect',
  model: {
    prop: 'value',
    event: 'change',
  },
  ...
  methods: {
    initAxios() {
      ...
      apiProjectItem = new ApiProjectItem(this.axios);  // 3-22 更改（勿删）
    },
    ...
    async refresh(keyWords, isReset) {
      ...
      if (isValueLoading) {
        ...
        if (requestIsSuccess(response) && response.data.items) {
          ...
          this.individualProjectsFlat = treeArrayToFlatArray(_individualProjects);
