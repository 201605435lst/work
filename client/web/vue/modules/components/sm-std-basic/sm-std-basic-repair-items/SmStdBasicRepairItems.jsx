...
export default {
  ...
  methods: {
    ...
    //上下移动
    async upOrDownAction(record, action) {
      if (action == "down" && record.order > 0) {
      ...
      let result=this.sortByOrder(this.repairItems);
      this.repairItems=result;

      let response = await apiRepairTestItem.updateOrder(data);
    ...
    sortByOrder(array) {
      array.map(item=>{
        if(item && item.repairTestItems){
          item.repairTestItems.sort((a, b) => a.order - b.order);
        }
      });
      return array;
    },
    ...
    async refresh(resetPage = true, page) {
      ...
      if (requestIsSuccess(response) && response.data) {
        ...
        this.repairItems=this.sortByOrder(result);
    ...
    return (
      <div class="sm-std-basic-repair-items">
        ...
        <a-table
          ...
          rowSelection={{
            columnWidth: 30,
            selectedRowKeys: this.selectUpgradeRepairIds,
            ...
            getCheckboxProps: record => {
              return {
                props: {
                  defaultChecked: this.selectUpgradeRepairIds.includes(record.id),
                },
              };
            },
          }}
          {...{
            scopedSlots: {
              ...
              organizationType: (text, record) => {
                let value = record.repairItemRltOrganizationTypes.map(item => item.organizationType.name)
                  .join('，');
                return (
                      ...
                      scopedSlots: {
                        ...
                        order: (text, record) => {
                          let value =
                            ...
                            <div class="arrow_up_down" >
                              <span class="text_up">{text}</span>
                              {vIf(
                                <span class="up_down">
                                  <div><a-icon type="arrow-up" onClick={() => this.upOrDownAction(record, 'up')} /></div>
                                  <div><a-icon type="arrow-down" onClick={() => this.upOrDownAction(record, 'down')} /></div>
                                </span>,
                                vP(this.permissions, permissionsSmStdBasic.RepairTestItems.UpdateOrder),
                              )}
