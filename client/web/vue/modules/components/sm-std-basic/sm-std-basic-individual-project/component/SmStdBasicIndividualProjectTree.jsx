...
export default {
  name: 'SmStdBasicIndividualProjectTree',
  props: {
  ...
  methods: {
    ...
    async loadByParentId(data) {
      this.expandedKeys = [];
      let response = await apiIndividualProject.getList({
        parentId: null,
        ids: [],
    ...
    // //根据关键字加载页面
    // async loadById(data) {
    //   let response = await apiIndividualProject.getTreeListByKeyWord(data);
    //   if (requestIsSuccess(response) && response.data) {
    //     let _individualProjects = treeArrayItemAddProps(response.data, 'children', [
    //       { sourceProp: 'name', targetProp: 'title' },
    //       { sourceProp: 'id', targetProp: 'value' },
    //       { sourceProp: 'id', targetProp: 'key' },
    //       { sourceProp: 'id', targetProp: 'key' },
    ...
    add(record) {
      this.record = record;
      this.$refs.SmStdBasicIndividualProjectModal.add(record);
    },
    // 查看
    view(record) {
    ...
    edit(record) {
      this.record = record;
      this.$refs.SmStdBasicIndividualProjectModal.edit(record);
    },
    // 删除
    remove(record) {
      this.record = record;
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise(async (resolve, reject) => {
            let response = await apiIndividualProject.delete(record.id);
        ...
        if (data == 'Add') {
          ...
          if (this.record != null) {
            ...
            if (this.record && this.record.children == null) {
              this.putIndividualProjectChildren(this.individualProjects);
            }
          ...
          // 查询当前记录,进行修改
          let response = await apiIndividualProject.get(data.id);
          if (requestIsSuccess(response) && response.data) {
            let dataResult = response.data;
            await this.editTreeData(this.individualProjects, dataResult);
          }
        }
      }
      //删除数据之后的页面刷新
      if (data == undefined) {
        this.getIndividualProjects(this.individualProjects);
        this.delateIndividualProjectsChildren(this.individualProjects, this.parentId);
        this.$emit('record', null);
      }
    },
    async refreshByParentId(data) {
      this.getRecordByParentId(this.individualProjects, data);
      let valueSourse = {};
      if (this.getRecord) {
      ...
      try {
        array.forEach((item, index, arr) => {
          ...
          if (item.children != null) {
            this.getIndividualProjects(item.children);
          }
        });
      } catch (e) {
        if (e.message != 'error') throw e;
    ...
    // 给添加的根节点的树添加一个孩子节点
    putIndividualProjectChildren(array) {
      try {
        array.forEach((item, index, arr) => {
          if (item.key == this.record.key) {
            item.children = [];
          ...
          if (item.children != null) {
            this.putIndividualProjectChildren(item.children);
          }
        });
      } catch (e) {
        if (e.message != 'error') throw e;
    ...
    delateIndividualProjectsChildren(array, id) {
      try {
        array.forEach((item, index, arr) => {
          if (item.id == id) {
          ...
          if (item.children != null) {
            this.delateIndividualProjectsChildren(item.children, id);
          }
        });
      } catch (e) {
        if (e.message != 'error') throw e;
  ...
  render() {
    return (
      <div class="sm-std-basic-individual-project-tree">
        <a-card title="单项工程列表" class="tree-card">
          <a
            slot="extra"
            onClick={() => {
              this.expandedKeys = [];
          ...
          {vIf(
            ...
            vP(this.permissions, permissionsSmStdBasic.IndividualProjects.Create),
          )}
          <div class="tree-body">
            <div class="std-tree">
                          ...
                          >
                            ...
                            <span class="icons">
                              {vIf(
                                ...
                                vP(
                                  this.permissions,
                                  permissionsSmStdBasic.IndividualProjects.Create,
                                ),
                              )}
                              ...
                              {vIf(
                                ...
                                vP(
                                  this.permissions,
                                  permissionsSmStdBasic.IndividualProjects.Update,
                                ),
                              )}
                              {vIf(
                                <span
                                ...
                                vP(
                                  this.permissions,
                                  permissionsSmStdBasic.IndividualProjects.Delete,
                                ),
                              )}
                            </span>
                          </div>,
              ...
              <div class="importOrExport">
                {vIf(
                  <div class="import">
                    <SmImport
                      ref="smImport"
                      url="api/app/stdBasicIndividualProject/upload"
                      axios={this.axios}
                  ...
                  vP(this.permissions, permissionsSmStdBasic.IndividualProjects.Import),
                )}
                {vIf(
                  <SmExport
                    ref="smExport"
                    url="api/app/stdBasicIndividualProject/export"
                    defaultTitle="导出"
                    axios={this.axios}
                    templateName="IndividualProject"
                    downloadFileName="单项工程表"
                    rowIndex={5}
                    onDownload={para => this.downloadFile(para)}
                  ></SmExport>,
                  vP(this.permissions, permissionsSmStdBasic.IndividualProjects.Export),
                )}
              </div>
              {vIf(
                <SmTemplateDownload
                  width="290px"
                  axios={this.axios}
                  downloadKey="individualProject"
                  downloadFileName="单项工程模板表"
                ></SmTemplateDownload>,
                vP(this.permissions, permissionsSmStdBasic.IndividualProjects.Import),
              )}
            </div>
