...
export default {
  ...
  methods: {
    ...
    async generatedCode() {
      let num = '001';
      let response = await apiComponentCategory.getListCode(this.datas ? this.datas.id : null);
              ...
              productName: (text, record, index) => {
                ...
                if (record != null) {
                  if (record.componentCategoryRltProductCategories.length > 0) {
                    result =
                      record.componentCategoryRltProductCategories[0].productionCategory.name;
                  }
                }
                return result ? (
                  <a-tooltip title={result} placement="topLeft">
