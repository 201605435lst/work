...
export default {
  name: 'SmStdBasicQuotaCategoryTreeSelect',
  model: {
    prop: 'value',
    event: 'change',
  },
  ...
  methods: {
    ...
    isValueLoading() {
      ...
      if (this.value instanceof Array) {
        if (this.value.length > 0) {
          // 保证数组里面的所有数据已经加载
          if (this.value.some(id => this.quotaCategorysFlat.find(x => x.id == id) == null)) {
            refresh = true;
          }
        } else {
          if (this.value.length == 0 && this.quotaCategorysFlat.length == 0) {
            refresh = true;
          }
        }
      }
      // 当是单选的时候
        ...
        if (this.value) {
          if (this.quotaCategorysFlat.find(x => x.id === this.value) == null) {
            refresh = true;
          }
        } else {
          if (!this.value && this.quotaCategorysFlat.length == 0) {
            refresh = true;
          }
        }
      }
    ...
    async refresh(keyWords, isReset) {
      ...
      if (isValueLoading) {
        ...
        if (requestIsSuccess(response) && response.data.items) {
          ...
          this.quotaCategorysFlat = treeArrayToFlatArray(_quotaCategorys);
    ...
    async onLoadData(treeNode) {
      if (treeNode.dataRef.children && treeNode.dataRef.children.length == 0) {
        ...
        if (requestIsSuccess(response) && response.data.items) {
          this.quotaCategorysFlat = this.quotaCategorysFlat.concat(response.data.items);
    ...
    async setValue() {
      let result = await this.processData(this.quotaCategorysFlat, this.value);
      if (result) {
        if (this.treeCheckable) {
