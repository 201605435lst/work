...
export default {
  name: 'SmStdBasicProcessTemplateTree',
  props: {
  ...
  methods: {
    ...
    //根据关键字加载页面
    async loadById(data) {
      let response = await apiProcessTemplate.getTreeListByKeyWord(data);
      if (requestIsSuccess(response) && response.data) {
        let _processTemplates = treeArrayItemAddProps(response.data, 'children', [
          { sourceProp: 'name', targetProp: 'title' },
          { sourceProp: 'id', targetProp: 'value' },
          { sourceProp: 'id', targetProp: 'key' },
          { sourceProp: 'id', targetProp: 'key' },
    ...
    add(record) {
      this.record = record;
      this.$refs.SmStdBasicProcessTemplateModal.add(record);
    },
    // 查看
    view(record) {
    ...
    edit(record) {
      this.record = record;
      this.$refs.SmStdBasicProcessTemplateModal.edit(record);
    },
    // 删除
    remove(record) {
      this.record = record;
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise(async (resolve, reject) => {
            let response = await apiProcessTemplate.delete(record.id);
        ...
        if (data == 'Add') {
          ...
          if (this.record != null) {
            ...
            if (this.record && this.record.children == null) {
              this.putIndividualProjectChildren(this.processTemplates);
            }
          ...
          // 查询当前记录,进行修改
          let response = await apiProcessTemplate.get(data.id);
          if (requestIsSuccess(response) && response.data) {
            let dataResult = response.data;
            await this.editTreeData(this.processTemplates, dataResult);
          }
        }
      }
      //删除数据之后的页面刷新
      if (data == undefined) {
        this.getProcessTemplates(this.processTemplates);
        this.delateProcessTemplatesChildren(this.processTemplates, this.parentId);
        this.$emit('record', null);
      }
    },
    async refreshByParentId(data) {
      this.getRecordByParentId(this.processTemplates, data);
      let valueSourse = {};
      if (this.getRecord) {
          ...
          if (array.some(item => item.id == data.id)) {
            array.map(item => {
              if (item.id == data.id) {
                ...
                item.prepositionId = data.prepositionId;
                item.code = data.code;
                item.name = data.name;
                item.unit = data.unit;
                ...
                item.durationUnit = data.durationUnit;
              }
            });
            throw new Error('error');
          }
      ...
      try {
        array.forEach((item, index, arr) => {
          ...
          if (item.children != null) {
            this.getProcessTemplates(item.children);
          }
        });
      } catch (e) {
        if (e.message != 'error') throw e;
    ...
    delateProcessTemplatesChildren(array, id) {
      try {
        array.forEach((item, index, arr) => {
          if (item.id == id) {
          ...
          if (item.children != null) {
            this.delateProcessTemplatesChildren(item.children, id);
          }
        });
      } catch (e) {
        if (e.message != 'error') throw e;
  ...
  render() {
    return (
      <div class="sm-std-basic-process-template-tree">
        <a-card title="工序模板列表" class="tree-card">
          <a
            slot="extra"
            onClick={() => {
              this.expandedKeys = [];
          ...
          {vIf(
            ...
            vP(this.permissions, permissionsSmStdBasic.ProcessTemplates.Create),
          )}
          <div class="tree-body">
            <div class="std-tree">
                          ...
                          >
                            ...
                            <span class="icons">
                              ...
                              {vIf(
                                ...
                                vP(this.permissions, permissionsSmStdBasic.ProcessTemplates.Delete),
                              )}
                            </span>
                          </div>,
                        ];
              ...
              <div class="importOrExport">
                {vIf(
                  <div class="import">
                    <SmImport
                      ref="smImport"
                      url="api/app/stdBasicProcessTemplate/upload"
                      axios={this.axios}
                  ...
                  vP(this.permissions, permissionsSmStdBasic.ProcessTemplates.Import),
                )}
                {vIf(
                  <SmExport
                    ref="smExport"
                    url="api/app/stdBasicProcessTemplate/export"
                    defaultTitle="导出"
                    axios={this.axios}
                    templateName="processTemplate"
                    downloadFileName="工序模板表"
                    rowIndex={5}
                    onDownload={para => this.downloadFile(para)}
                  ></SmExport>,
                  vP(this.permissions, permissionsSmStdBasic.ProcessTemplates.Export),
                )}
              </div>
              {vIf(
                <SmTemplateDownload
