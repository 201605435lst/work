...
import SmStdBasicComponentCategoryTreeSelect from '../sm-std-basic-component-category-tree-select';
  ...
  methods: {
    ...
    async edit(record) {
      ...
      if (utils.requestIsSuccess(response)) {
        ...
        this.serviceLifeUnit = this.record.serviceLifeUnit
          ? this.record.serviceLifeUnit
          : ServiceLifeUnit.Year;
        ...
        this.form.validateFields(async (err, values) => {
          if (!err) {
            ...
            if (this.status === ModalStatus.Add) {
              response = await apiStandardEquipment.create({
                ...values,
        ...
        <a-form form={this.form}>
          ...
          >
            <SmStdBasicProductCategoryTreeSelect
              ...
              placeholder={this.status === ModalStatus.View ? '' : '请选择产品分类'}
              disabled={this.status === ModalStatus.View}
              v-decorator={[
                'productCategoryId',
                {
          ...
          >
            <SmStdBasicComponentCategoryTreeSelect
              ...
              showSearch={true}
              parentDisabled={true}
              ref="SmStdBasicComponentCategoryTreeSelect"
          ...
          >
            <SmStdBasicManufacturerSelect
              axios={this.axios}
              placeholder={this.status == ModalStatus.View ? '' : '请选择产品厂家'}
              disabled={this.status == ModalStatus.View}
              height={32}
              v-decorator={[
                'manufacturerId',
                {
                  initialValue: null,
                  rules: [{ required: true, message: '请选择产品厂家！' }],
                },
              ]}
            />
          </a-form-item>
          ...
          >
            <a-input
              disabled={this.status == ModalStatus.View}
              placeholder={this.status == ModalStatus.View ? '' : '请输入铁总编码'}
              v-decorator={[
                'csrgCode',
                {
                  initialValue: '',
          ...
          >
            <a-input-group compact={true} style="width:100%">
              ...
              >
                <a-select-option value={ServiceLifeUnit.Year}>
                  {utils.getServiceLifeUnit(ServiceLifeUnit.Year)}
                </a-select-option>
                <a-select-option value={ServiceLifeUnit.Month}>
                  {utils.getServiceLifeUnit(ServiceLifeUnit.Month)}
                </a-select-option>
                <a-select-option value={ServiceLifeUnit.Day}>
                  {utils.getServiceLifeUnit(ServiceLifeUnit.Day)}
                </a-select-option>
              </a-select>
            </a-input-group>
          </a-form-item>
