  ...
  methods: {
    ...
    async refresh() {
      ...
      if (requestIsSuccess(response)) {
        let lala= JSON.stringify(response.data);
        let newData =  JSON.parse(lala);
        let _categories = treeArrayItemAddProps(response.data.items, 'children', [
          {
            targetProp: 'title',
            handler: item => {
              let result = item.title.length > 10 ? `${item.title.substring(0, 10)}...` : item.title;
              return <div title={item.title}>{result}</div>;
            },
          },
