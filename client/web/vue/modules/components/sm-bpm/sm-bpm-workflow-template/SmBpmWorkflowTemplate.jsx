  ...
  data() {
    return {
      ...
      nodeConfig: [
        ...
        { type: 'bpmApprove', width: 80, height: 44, nodeProps: ["basic", "member-select", "field-permission","code"] },
  ...
  methods: {
    ...
    async refresh() {
      ...
      if (requestIsSuccess(response) && response.data) {
        ...
        // 检查当前流程类型
        this.single=this.workflow.type===1?true:false;
        // 校验数据合法
        let formTemplate =
        ...
        if (this.iPageState === PageState.Edit) {
          ...
          this.flowData = flowTemplate
            ...
            : initialFlowData;
          this.$nextTick(() => {
            if (!this.single && this.$refs.smBpmFormDesign !== undefined) {
              this.$refs.smBpmFormDesign.data = this.formData;
            }
        ...
        if (this.iPageState === PageState.Add) {
          ...
          if (requestIsSuccess(response)) {
            ...
            response = await apiWorkflowTemplate.updateFormTemplateAndFlowTemplate({
              id: this.workflow.id,
              ...{
                formItems: JSON.stringify(this.single?[]:this.$refs.smBpmFormDesign.data.list),
                formConfig: JSON.stringify(this.single?[]:this.$refs.smBpmFormDesign.data.config),
    ...
    onTabChange(value) {
      ...
      if ((value === 3 && !this.workflow.published)||(value===2&&this.single&&!this.workflow.published)) {
        // 检查是否发布
        this.$confirm({
          title: '提示',
  ...
  render() {
    return (
      <div class={`sm-bpm-workflow-template panel ${this.border ? 'border' : ''}`}>
        <a-tabs animated={true} activeKey={this.single?this.activeKey:this.activeKey} onChange={this.onTabChange}>
          ...
          <a-tab-pane key={this.single?1:2} force-render>
            <span slot="tab">
              <span class="step-order">{this.single?1:2}</span>
          ...
          <a-tab-pane key={this.single?2:3} force-render disabled={this.formDataChanged || this.flowDataChanged}>
            <span slot="tab">
              <span class="step-order">{this.single?2:3}</span>
