...
export default {
  ...
  props: {
    ...
    isBack: { type: Boolean, default: true },//是否显示退回按钮(高铁科添加：取消退回按钮)
    isYunWei: { type: Boolean, default: false },//是否显示根据提交审批类型筛选
    submitType: { type: Boolean, default: false },
  },
  data() {
    return {
      workflows: [],
  ...
  methods: {
    ...
    async refresh(resetPage = true) {
      ...
      let _params = {
        ...
        submitStartTime: moment(this.submitTime)
          .date(1)
          .hours(0)
          .minutes(0)
          .seconds(0)
          .format('YYYY-MM-DD HH:mm:ss'),
        submitEndTime: moment(this.submitTime)
          .endOf('month')
          .endOf('date')
          .format('YYYY-MM-DD HH:mm:ss'),
        ...this.queryParams,
      ...
      if (this.isYunWei && !this.submitType) {
        let getWorkflowNamesRsponse = await apiWorkflow.getWorkFlowNames();
        if (requestIsSuccess(getWorkflowNamesRsponse) && getWorkflowNamesRsponse.data) {
          this.workflowNames = getWorkflowNamesRsponse.data;
        }
      }
  ...
  render() {
    ...
    let workflowNameSelect = [];
    if (this.isYunWei && !this.submitType) {
      for (let item in this.workflowNames) {
        workflowNameSelect.push(
          <a-select-option key={this.workflowNames[item]}>
            {this.workflowNames[item]}
          </a-select-option>,
        );
      }
    }
    if (this.isYunWei && this.submitType) {
      workflowNameSelect.push(
        <a-select-option key="年表月度计划审核">{'年表月度计划审核'}</a-select-option>,
      );
      workflowNameSelect.push(
        <a-select-option key="计划变更审核">{'计划变更审核'}</a-select-option>,
      );
    }
                        ...
                        return (
                          <span class="info-item">
                            ...
                            {imgs instanceof Array
                              ? imgs.map(a => {
                                return (
                                  <SmFileTag url={a.url} fileType={a.type} fileName={a.name} />
                                );
              ...
              operations: (text, record) => {
                return (
                  <span>
                    ...
                    >
                      {/* 如果是系统流程，重启发起按钮禁用 */}
                      {this.queryParams.group === UserWorkflowGroup.Waiting
                        ? '审批'
                        : record.state === WorkflowState.Stopped &&
                          this.queryParams.group == UserWorkflowGroup.Initial
                          ? '重新发起'
                          : '详情'}
                    </a>
