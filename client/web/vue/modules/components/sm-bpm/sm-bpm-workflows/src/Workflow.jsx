...
import SmCrPlanSkylightPlanWorkTicketModal from "../../../sm-cr-plan/sm-cr-plan-skylight-plans/SmCrPlanSkylightPlanWorkTicketModal";
...
export default {
  ...
  methods: {
    ...
    async refresh() {
      ...
      //高铁需求：在特定节点添加显示按钮(后期优化 改成递归形式)
      if (!this.isBack && this.workflow.isStatic) {
        let endSourceNodeId = null;
        let nodeIds = [];
          ...
          if (node.type == 'bpmEnd') {
            this.flowData.edges.forEach(item => {
              if (item.target == node.id) {
                endSourceNodeId = item.source; //查询到倒数第一条线
              }
            });
          }
        });
        this.flowData.nodes.forEach(node => {
          if (node.id == endSourceNodeId) {
            this.flowData.edges.forEach(edge => {
              if (edge.target == node.id) {
                //查询到倒数第二条线
                this.flowData.nodes.forEach(item => {
                  if (item.id == edge.source && item.active == true) {
                    this.canNodeClick = true;
                  }
                });
              }
            });
    ...
    //垂直天窗计划
    palnDetails() {
      this.$refs.SmCrPlanSkylightPlanWorkTicketModal.view(this.id);
    },
  },
  render() {
    return (
                ...
                <div class="btns">
                  ...
                  <a-button
                    ...
                    onClick={() => {
                      let _this = this;
                      this.$confirm({
                        title: tipsConfig.refuse.title,
                        content: h => <div style="color:red;">{tipsConfig.refuse.content}</div>,
                        okType: 'danger',
                        onOk() {
