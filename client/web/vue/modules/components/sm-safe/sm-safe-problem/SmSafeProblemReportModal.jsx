...
export default {
  ...
  methods: {
    ...
    async refresh() {
      let response = await apiProblem.getSafeProblemReport(this.record.id);
    ...
    print() {
      printJS({
        printable: 'sm-safe-problem-report-modal',
        type: 'html',
        maxWidth: '100%',
        targetStyles: ['*'],
        ...
        _file.push(
          <span class="file" onClick={() => {
            ...
            {this.imgtypes.includes(item.file.type) ?
              ...
              :
              <a-tag>{item.file.name + item.file.type}</a-tag>
            }
          </span>,
        );
      });
    ...
    safeProblemReport() {
      let _safeProblem = [];
      this.record && this.record.safeProblemRecord && this.record.safeProblemRecord.map((item, index) => {
        _safeProblem.push(
          <div style="margin-top: 20px;">
            ...
            <a-row class={{
              "rows": true,
              "file-row": true,
              "row-bottom": index == this.record.safeProblemRecord.length - 1,
            }}>
              <a-col span="3" class="title">
                                附件
              </a-col>
  ...
  render() {
    ...
    return (
      <a-modal
        ...
        width={1000}
      >
        <div class="sm-safe-problem-report-modal" >
          <div id="sm-safe-problem-report-modal">
            <a-row>
            ...
            <a-row class="rows">
              ...
              <a-col span="9" class="longText">
                <span class="text">{this.record && this.record.type ? this.record.type.name : ''}</span>
              </a-col>
            </a-row>
            <a-row class="rows">
              <a-col span="3" class="title">
            ...
            <a-row class={{
              "rows": true,
              "file-row": true,
              "row-bottom":this.record && this.record.safeProblemRecord && this.record.safeProblemRecord.length ==0,
            }}>
              <a-col span="3" class="title">
                                附件
              </a-col>
