...
import SmCostmanagementOtherCostModal from './SmCostmanagementOtherCostModal';

import moment from 'moment';
let apiCostOther = new ApiCostOther();

export default {
  name: 'SmCostmanagementOtherCost',
  props: {
  ...
  methods: {
    ...
    // 添加
    add() {
      this.$refs.SmCostmanagementOtherCostModal.add();
    },

    // 编辑
    edit(record) {
      this.$refs.SmCostmanagementOtherCostModal.edit(record);
    },
    // 删除
        ...
        this.$confirm({
          ...
          onOk() {
            return new Promise(async (resolve, reject) => {
              let response = await apiCostOther.delete(selectedIds);
    ...
    async refresh(resetPage = true, page) {
      ...
      let response = await apiCostOther.getList({
  ...
  render() {
    return (
      <div class="sm-costmanagement-other-cost">
        <sc-table-operator
          onSearch={() => {
            this.refresh();
          }}
        ...
        >
          ...
          <div class="costmanagement-other-cost-date">
          ...
          <template slot="buttons">
            {vIf(
              ...
              vP(this.permissions, permissionsCostmanagement.CostOthers.Create),
            )}
            {vIf( 
              <a-button type="danger"  onClick={() => this.remove(true, this.selectedRowKeys)}>
                删除
              </a-button>,
              vP(this.permissions, permissionsCostmanagement.CostOthers.Delete),
            )} 
          </template>
        </sc-table-operator>
        <a-table
          ...
          {...{
            scopedSlots: {
              ...
              type: (text, record) => {
                let name=record.type?record.type.name:'';
                return (
              ...
              creator: (text, record) => {
                return (
                  <a-tooltip placement="topLeft" title={record.creatorName}>
                ...
                return [
                  <span>
                    {vIf(
                      ...
                      vP(this.permissions, permissionsCostmanagement.CostOthers.Update),
                    )}
                    {vIf( 
        ...
        ></a-table>
        {/* 添加/编辑模板 */}
        <SmCostmanagementOtherCostModal
          ref="SmCostmanagementOtherCostModal"
          axios={this.axios}
          onSuccess={() => {
            this.refresh(false);
          }}
