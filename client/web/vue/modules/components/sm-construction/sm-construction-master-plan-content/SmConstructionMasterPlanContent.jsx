...
import SmConstructionMasterPlanContentModal from './SmConstructionMasterPlanContentModal';
import SmConstructionMasterPlanContentFrontSelectTreeModal
  from '../../sm-construction/sm-construction-master-plan-content/SmConstructionMasterPlanContentFrontSelectTreeModal';
...
export default {
  ...
  computed: {
    ...
    addBtnDisable() { // 添加按钮 是否禁用
      ...
      if (this.selectMasterPlanContentIds.length === 1) { // 如果 多选了也禁用
        return false;
      }
      return true;
    },
            ...
            this.clickTimeChange = setTimeout( // 因为双击事件会触发 单击事件,所以定义 一个定时器 ,当 0.3秒内 点击两次的话就不算单击啦
              () => {
                if (this.rowMultiple) { // 多选的话 就 往里面加
                  if (this.selectMasterPlanContentIds.includes(record.id)) { // 包含 往里面 删
                    this.selectMasterPlanContentIds = this.selectMasterPlanContentIds.filter(x => x !== record.id);
                  } else { // 不包含 就添加
                    this.selectMasterPlanContentIds.push(record.id);
                  }
                } else { // 单选就替换
                  this.selectMasterPlanContentIds = [record.id];
    ...
    columns() {
      let baseColumns = this.isModalState ?
        ...
        [
          {
            title: '任务名称', dataIndex: 'name', align: 'left', ellipsis: true, customRender: (text, record, index) => {
              return <span>{text}{record.isMilestone ?
                <a-tag style='margin-left:5px' color='blue'>里程碑</a-tag> : undefined}</span>;
            },
          },
          {
            title: '前置任务', dataIndex: 'antecedents', ellipsis: true, customRender: (text, record, index) => {
          ...
          {
            title: '开始时间', dataIndex: 'planStartTime', ellipsis: true, customRender: (text, record, index) => {
              return (<span>{moment(text).format('YYYY-MM-DD')}</span>);
            },
          },

          {
            title: '完成时间', dataIndex: 'planEndTime', ellipsis: true, customRender: (text, record, index) => {
              return (<span>{moment(text).format('YYYY-MM-DD')}</span>);
            },
          },
        ];
  ...
  methods: {
    ...
    switchSelectType() {
      ...
      this.selectMasterPlanContentIds = []; // 同时清空选择的东西
    },
    async selected(value) {
      console.log(value);
      let subItemContentIds = value.map(x => x.id);
      let res = await apiMasterPlanContent.import(this.selectId, subItemContentIds);
    ...
    async refresh() {
      ...
      this.loading = true;
      let res = await apiMasterPlanContent.getSingleTree(this.masterPlanId);
    ...
    add() {
      if (this.list.length === 0) { // 没有一个就新建 一个
        this.$refs.SmConstructionMasterPlanContentModal.add(this.masterPlanId);
      } else { // 否则就新建一个子级
        this.addChild({ id: this.selectMasterPlanContentIds[0] });
      }
    },
    // 添加子级
    addChild(record) {
      const contentId = record.id;
      this.$refs.SmConstructionMasterPlanContentModal.addChild(contentId);
    },
    // 升级
    async lvUp(id) {
      let res = await apiMasterPlanContent.lvUp(id);
      if (requestIsSuccess(res)) {
        await this.refresh();
      }
    },
    // 降级
    async lvDown(id) {
      let res = await apiMasterPlanContent.lvDown(id);
      if (requestIsSuccess(res)) {
        await this.refresh();
      }
    },
    // 添加(打开添加模态框) 123
    edit(record) {
      this.$refs.SmConstructionMasterPlanContentModal.edit(record);
    },
    async addFrontTask(record) {
      this.$refs.SmConstructionMasterPlanContentFrontSelectTreeModal.addFrontTask(record, this.list);
    },
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise(async (resolve, reject) => {
            const response = await apiMasterPlanContent.deleteRange(ids);
            _this.refresh();
  ...
  render() {
    return (
      <div>
        ...
        {/*        编辑*/}
        {/*      </a-button>,*/}
        {/*      <a-button size='small' type='danger' icon='' onClick={() => this.remove(this.selectMasterPlanContentIds)}*/}
        {/*        disabled={this.selectMasterPlanContentIds.length === 0}>*/}
        {/*        删除*/}
        {/*      </a-button>,*/}
        ...
        {this.list && this.list.length ?
          <a-table
            ...
            rowSelection={this.showSelectRow ? {
              selectedRowKeys: this.selectMasterPlanContentIds,
              // columnWidth: 30,
              type: this.rowMultiple ? 'checkbox' : 'radio',
              fixed: true,
              onChange: selectedRowKeys => {
                this.selectMasterPlanContentIds = selectedRowKeys;
              },
            } : undefined}
          />
          :
        ...
        {/*添加/编辑模板*/}
        <SmConstructionMasterPlanContentModal
          ref='SmConstructionMasterPlanContentModal'
          axios={this.axios}
          onSuccess={async () => {
            this.list = [];
            await this.refresh();
        ...
        />
        {/*添加前置任务-Modal*/}
        <SmConstructionMasterPlanContentFrontSelectTreeModal
          ref='SmConstructionMasterPlanContentFrontSelectTreeModal'
          axios={this.axios}
          onSuccess={async () => {
            this.list = [];
            await this.refresh();
