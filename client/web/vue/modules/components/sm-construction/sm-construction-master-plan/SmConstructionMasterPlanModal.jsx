...
export default {
  name: 'SmConstructionMasterPlanModal',
  props: {
      ...
      this.form.validateFields(async (err, values) => {
        if (!err) {
          ...
          if (this.status === ModalStatus.Add) {
            // 添加设备台班信息
            response = await apiMasterPlan.create({ ...values });
          } else if (this.status === ModalStatus.Edit) {
            // 添加设备台班信息
            response = await apiMasterPlan.update(this.record.id, { ...values });
          }
