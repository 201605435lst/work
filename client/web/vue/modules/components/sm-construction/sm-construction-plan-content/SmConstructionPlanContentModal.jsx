...
// 表单字段
const formFields = ['planId', 'name', 'content', 'planStartTime', 'planEndTime', 'period', 'parentId','isMilestone'];
export default {
  name: 'SmConstructionPlanContentModal',
  props: {
  ...
  methods: {
    ...
    // 添加
    add(PlanId) {
      if (!!PlanId) { // contentId  不为 null 或者 undefined 或者  ''
        this.PlanId = PlanId;
      }
      ...
      this.form.validateFields(async (err, values) => {
        if (!err) {
          ...
          if (this.status === ModalStatus.Add) {
            response = await apiPlanContent.create({ ...values }); // 添加任务计划详情
          } else if (this.status === ModalStatus.Edit) {
            response = await apiPlanContent.update(this.record.id, { ...values }); // 编辑任务计划详情
          }
