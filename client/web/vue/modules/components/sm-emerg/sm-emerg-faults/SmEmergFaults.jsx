...
export default {
  ...
  methods: {
    ...
    //处理流程
    process(record) {
      this.$emit('process', true, record.id,'normal');
    },
    ...
    //     {
    //       url: '/api/app/crPlanSkylightPlan/export',
    //       method: 'post',
    //       responseType:'blob',
    //     },
    //   ).then(response=>{
    ...
    //       FileSaver.saveAs(new Blob([response.data], { type: 'aapplication/msword' }), '测试.doc');
      ...
      <div class="sm-emerg-fault">
        ...
        >
          ...
          <a-form-item label="关键字" style={'margin-left: 14px'}>
            <a-input
              placeholder="故障现象、监测异常、原因分析、处理过程、备注"
