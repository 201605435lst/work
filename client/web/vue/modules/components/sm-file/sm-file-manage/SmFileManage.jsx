...
export default {
  ...
  computed: {
    ...
    doneApproveDataArray() {
      return this.doneApproveData;
    },
    staticDataArray() {
      return this.staticData[0].children;
    },
    shareDataArray() {
  ...
  async created() {
    ...
    if (this.staticKey) {
      ...
      this.staticTreeSelect(this.staticData.length > 0 ? this.staticData[0] : null);
    } else {
  ...
  methods: {
    ...
    // 回收站点击
    deleteClick() {
      this.isApprove ? this.$refs.ApproveTree.cancleSlectedState() : '';
      this.isApprove ? this.$refs.DoneApproveTree.cancleSlectedState() : '';
    ...
    //获取静态文件夹数据
    async getStaticData() {
      let response = await apiFileManage.getStaticTreeList(this.staticKey);
      if (requestIsSuccess(response) && response.data && response.data.length > 0) {
        this.staticData = response.data;
        this.staticData.children = TreeNodeAddIcon(response.data[0].children, 'folder');
      }
    },
    ...
    organizationTreeSelect(node) {
      this.isApprove ? this.$refs.ApproveTree.cancleSlectedState() : '';
      this.isApprove ? this.$refs.DoneApproveTree.cancleSlectedState() : '';
      this.isApprove || this.staticKey ? "" : this.$refs.ShareTree.cancleSlectedState();
      this.$refs.MineTree.cancleSlectedState();
      let params = {};
      if (node.type == 0) {
        params = {
    ...
    approveSelect(node) {
      this.isApprove ? this.$refs.DoneApproveTree.cancleSlectedState() : '';
      this.staticKey ? '' : this.$refs.MineTree.cancleSlectedState();
      this.staticKey ? '' : this.$refs.SmFileTree.cancleSlectedState();
      this.staticKey ? this.$refs.SmFileStaticTree.cancleSlectedState() : '';
      this.isApprove || this.staticKey ? '' : this.$refs.ShareTree.cancleSlectedState();
      let params = {
        id: node.id,
        type: 2,
      ...
      this.$refs.SmFileList.paramChange(params, ResourceTableType.Approve, true);
      this.selectedType = ResourceTableType.Approve;
      this.currentParams = params;
    },
    // "yi审批"树节点选中事件
    doneApproveSelect(node) {
      this.isApprove ? this.$refs.ApproveTree.cancleSlectedState() : '';
      this.staticKey ? '' : this.$refs.MineTree.cancleSlectedState();
      this.staticKey ? '' : this.$refs.SmFileTree.cancleSlectedState();
      this.staticKey ? this.$refs.SmFileStaticTree.cancleSlectedState() : '';
      ...
      this.$refs.SmFileList.paramChange(params, ResourceTableType.DoneApprove, true);
      this.selectedType = ResourceTableType.DoneApprove;
      this.currentParams = params;
    },
    // 共享中心树节点选中事件
    ...
    // 静态文件夹节点选中事件
    staticTreeSelect(node, selected) {
      this.isApprove ? this.$refs.ApproveTree.cancleSlectedState() : '';
      this.isApprove ? this.$refs.DoneApproveTree.cancleSlectedState() : '';
      if (!selected) {
        node = this.staticData[0];
      }

      let params = {
        id: node ? node.id : GuidEmpty,
        type: 2,
      };

      this.$refs.SmFileList.paramChange(params, ResourceTableType.Static, true);
      this.selectedType = ResourceTableType.Static;
      this.currentParams = params;
    ...
    let fileManageContent = (
      <div class="f-left-content">
        {this.staticKey ?
          ...
          [< SmFileTree
            class="f-static-tree"
            ref="SmFileStaticTree"
            selectedKeys={this.selectedTreeNodes}
            onSelect={this.staticTreeSelect}
            treeData={this.staticDataArray}
            onChange={v => {
              this.selectedTreeNodes == v;
            }}
          />,
          <div class="f-share-content" style={{ maxHeight: this.height * 0.3 + 'px' }}></div>]
          : [
          ...
          <div class="f-panel">
            ...
            {this.panelIndex == 1 ? (
              <div class="f-panel-item" style={{ display: this.panelIndex == 1 }}>
                {fileManageContent}
                {this.staticKey || this.isApprove ? undefined :
                ...
                {this.isApprove ?
                  [<SmFileTree
                    ref="ApproveTree"
                    onSelect={this.approveSelect}
                    selectedKeys={this.selectedTreeNodes}
                    treeData={this.approveDataArray}
                    onChange={v => {
                      this.selectedTreeNodes == v;
                    }}
                  />,
                  <SmFileTree
                    ref="DoneApproveTree"
                    onSelect={this.doneApproveSelect}
                    selectedKeys={this.selectedTreeNodes}
                    treeData={this.doneApproveDataArray}
                    onChange={v => {
                      this.selectedTreeNodes == v;
                    }}
                  />] : ''}
        ...
        <div class="f-right">
          ...
          {this.panelIndex == 1 ? (
            <SmFileList
              ...
              topFolder={this.staticData.length > 0 ? this.staticData[0] : null}
              axios={this.axios}
              onRefresh={() => {
                if (this.staticKey) {
                  this.getStaticData();
