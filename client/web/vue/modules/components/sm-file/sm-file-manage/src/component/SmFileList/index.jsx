...
export default {
  ...
  props: {
    ...
    topFolder: { type: Object, default: null }, //顶级文件夹
  },
  data() {
    return {
      ...
      queryObj: null,
      tagList: [],
      toolbar: this.staticKey ? MenuNames.filter(a => a.name === 'newFolder') : MenuNames.filter(a => a.name === 'upload' || a.name === 'uploadD'), // 工具栏的菜单
  ...
  methods: {
    ...
    paramChange(params, tableType, treeNodeClick) {
      ...
      if (tableType !== ResourceTableType.Static) {
        this.getTagList();
      }
    ...
    async createFolder() {
      ...
      if (this.tableType == ResourceTableType.Organization) {
        ...
        obj = {
          organizationId: null,
          parentId: this.queryObj ? this.queryObj.id : GuidEmpty,
        };
      }
      this.$refs.SmResourceList.createFolder({ ...obj, staticKey: this.staticKey });
    },
    ...
    async getToolBarMenus() {
      ...
      case ResourceTableType.Static:
        this.toolbar = MenuNames.filter(a => ['upload', 'newFolder', 'downLoad', 'delete'].includes(a.name));
        break;
      case ResourceTableType.Approve:
        this.toolbar = MenuNames.filter(a => a.name == 'downLoad');
        break;
