...
import { ResourceTableType, ApprovalStatus } from '../../../../../_utils/enum';
...
import SmFileApproveModal from './SmFileApproveModal';
...
export default {
  ...
  props: {
    ...
    staticKey: { type: String, default: '' }, // 静态文件夹key值
    isApprove: { type: Boolean, default: false }, //是否审批模式
  },
  data() {
    return {
  ...
  methods: {
    ...
    async queryResource(keyWords, tagId) {
      ...
      if (this.currentParams != null) {
        let data = {
          staticKey: this.staticKey ? this.staticKey : '',
          name: keyWords,
          nodeId: this.currentParams.id,
          nodeType: this.currentParams.id === undefined ? 2 : this.currentParams.type - 1, // 默认参数中的类型时为了查询资源，查询文件需要修改类型
    ...
    getMenusState() {
      ...
      case ResourceTableType.Organization:
        if (this.isApprove) {
          menus = MenuNames.filter(a => a.listMenu && a.name != 'publish' && a.name != 'approve');
        } else {
          menus = MenuNames.filter(a => a.listMenu && a.name != 'publish' && a.name != 'approveFlow' && a.name != 'approve' && a.name != 'approveFlowView');
        }
        break;
      case ResourceTableType.Mine:
        if (this.isApprove) {
          menus = MenuNames.filter(a => ['publish', 'permission', 'delete', 'approveFlow', 'approveFlowView'].includes(a.name));
        } else {
          menus = MenuNames.filter(a => ['publish', 'permission', 'delete'].includes(a.name));
        }
        break;
      case ResourceTableType.DoneApprove:
        menus = MenuNames.filter(a => a.name == 'approveFlowView');
        break;
      case ResourceTableType.Approve:
        menus = MenuNames.filter(a => a.name == 'approve');
        break;
      ...
      case ResourceTableType.ShareCenter:
        ...
        break;
      case ResourceTableType.Static:
        if (this.isApprove) {
          menus = MenuNames.filter(a => ['downLoad', 'rename', 'delete', 'approveFlow', 'approveFlowView'].includes(a.name));
        } else {
          menus = MenuNames.filter(
            a => a.listMenu && ['downLoad', 'rename', 'delete'].includes(a.name),
          );
        }
    ...
    getMenuItems(row) {
      ...
      if (this.currentType === ResourceTableType.ShareCenter) {
        let limit = ['tag', 'version', 'permission', 'share', 'move','approveFlow', 'approveFlowView', "approve"];
        menus = menus.filter(a => !limit.includes(a.name));
      }
      if (row && this.isApprove && (row.status == ApprovalStatus.OnReview || row.status == ApprovalStatus.Pass)) {
        menus = menus.filter(a => a.name != 'approveFlow');
      }
      if (row && this.isApprove && (row.status == ApprovalStatus.ALL || row.status == ApprovalStatus.ToSubmit)) {
        menus = menus.filter(a => a.name != 'approveFlowView');
      }
      if (row && this.isApprove && row.resourceType === ResourceType.Folder) {
        let _limit = ['approveFlow', 'approveFlowView', "approve"];
        menus = menus.filter(a => !_limit.includes(a.name));
      }
    ...
    operationEvent(item, row) {
      ...
      case 'approveFlow':
        ...
        this.submit(row);
        break;
      case 'approveFlowView':
        // 流程查看
        this.workflowView(row);
        break;
      default:
        break;
      }
      ...
      oId =
        this.currentParams.type == 1
          ...
          : await GetOraganizationId(
            this.axios,
            this.selectedList[0].resourceType,
    ...
    async editSave(row) {
      ...
      if (this.newFolderParams != null) {
        ...
        let param = {
          ...
          name: row.name,
          staticKey: this.newFolderParams.staticKey,
        };
        ...
        async onOk() {
          ...
          if (requestIsSuccess(response)) {
            if (_this.currentType === ResourceTableType.Delete) {
              ...
              _this.$message.success(_this.currentType == ResourceTableType.Static ? '资源已删除' : '资源已删除，可在回收站找回');
            }
    ...
    view(row) {
      ...
      } else if (videoTypes.includes(row.type)) {
        this.$refs.smVideo.preview(true, getFileUrl(row.url), row.name);
      }
      else {
    ...
    approve(record) {
      this.record = record;
      this.$refs.SmFileApproveModal.approve(record);
    },
    // 流程模板选择回调
    async flowSelected(res) {
      let data = {
        workflowTemplateId: res.id,
        fileId: this.commitSelected.id,
      };
      // 确认流程，并提交
      let response = await apiFileApprove.create(data);
      if (requestIsSuccess(response)) {
        if (response.data) {
            ...
            scopedSlots: {
              ...
              status: (text, record, index) => {
                ...
                case ApprovalStatus.OnReview:
                  return <a-tag size="small" color="#faad14">审核中</a-tag>;
                case ApprovalStatus.Pass:
                  return <a-tag size="small" color="#61D023">已审核</a-tag>;
                case ApprovalStatus.UnPass:
                  return <a-tag size="small" color="#f50">已驳回</a-tag>;
                default:
                  return "";
                }
              },
