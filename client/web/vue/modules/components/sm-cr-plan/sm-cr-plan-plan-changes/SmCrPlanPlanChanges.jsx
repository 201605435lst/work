...
import { YearMonthPlanState, SelectablePlanType, RepairTags } from '../../_utils/enum';
...
export default {
  ...
  props: {
    ...
    orgKey: { type:String,default:null},
  },
  data() {
    return {
      changes: [], // 列表数据源
  ...
  methods: {
    ...
    async export(record) {
      ...
      switch (this.orgKey) {
      case RepairTags.RailwayHighSpeed:
        signatureName = "gtkSignature.png";
        break;
      case RepairTags.RailwayWired:
        signatureName = "yxkSignature.png";
        break;
      default:
        signatureName = "wxkSignature.png";
        break;
      }
      ...
      if (requestIsSuccess(foreResp)) {
        let fileName =
          ...
          '变更单.xls';
        let response = await apiAlterRecord.export(record.id, signatureName);
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise(async (resolve, reject) => {
            let response = await apiAlterRecord.delete(record.id, _this.repairTagKey);
            _this.refresh();
    ...
    async refresh(resetPage = true) {
      ...
      this.queryParams.planStartTime = moment(this.queryParams.planTime)
        .date(1)
        .hours(0)
        .minutes(0)
        .seconds(0)
      ...
      this.queryParams.planEndTime = moment(this.queryParams.planTime)
        .endOf('month')
        .format('YYYY-MM-DD HH:mm:ss');

      this.queryParams.alterStartTime = moment(this.queryParams.alterTime)
        .date(1)
        .hours(0)
        .minutes(0)
        .seconds(0)
      ...
      this.queryParams.alterEndTime = moment(this.queryParams.alterTime)
        .endOf('month')
        .format('YYYY-MM-DD HH:mm:ss');
      ...
      <div class="sm-cr-plan-plan-changes">
        ...
        >
          ...
          <a-form-item label="变更时间">
            <a-month-picker
              style="width:100%"
              value={this.queryParams.alterTime}
              onChange={value => {
                this.queryParams.alterTime = value;
                this.refresh();
              }}
              allowClear={false}
            ></a-month-picker>
                      ...
                      <a-dropdown trigger={['click']}>
                        ...
                        <a-menu slot="overlay">
                          {record.state === YearMonthPlanState.UnCommit ||
                            record.state === YearMonthPlanState.UnPassed ? (
                              <a-menu-item>
                                {vIf(
                                  <a
                                    onClick={() => {
