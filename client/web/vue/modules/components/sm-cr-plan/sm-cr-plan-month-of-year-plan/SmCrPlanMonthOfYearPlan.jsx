...
import SmCrPlanMonthOfYearPlanModal from './SmCrPlanMonthOfYearPlanModal';
...
export default {
  ...
  computed: {
    ...
    columns() {
      ...
      return [
        ...
        this.repairTagKey === 'RepairTag.RailwayWired'
          ? {
            title: '编制执行单位',
            width: 120,
  ...
  methods: {
    ...
    edit(record) {
      this.$refs.SmCrPlanMonthOfYearPlanModal.edit(record, this.monthsDay);
    },
    async export(record) {
      //导出按钮
      if (!this.queryParams.orgId) {
    ...
    backPlans() {
      ...
      this.$confirm({
        title: '重要提示',
        content: h => <div style="color:red;">确认退回该车间的年表月度计划?此过程不可逆!</div>,
        okType: 'danger',
        onOk() {
          return new Promise(async (resolve, reject) => {
            let data = {
              OrgId: _this.queryParams.orgId,
              Year: _this.queryParams.year,
              Month: _this.queryParams.month,
              RepairTagKey: _this.repairTagKey,
            };
            let response = await apiYearMonthPlan.backMonthOfYearPalns(data);
            if (requestIsSuccess(response)) {
              _this.$message.info('退回成功');
              _this.refresh();
            }
            setTimeout(requestIsSuccess(response) ? resolve : reject, 100);
          });
  ...
  render() {
    ...
    return (
      <div>
          ...
          <template slot="buttons">
            <div style="display:flex;">
              {vIf(
                ...
                vP(this.permissions, permissionsSmCrPlan.MonthOfYearPlan.Create),
              )}
              {vIf(
              ...
              {
                this.monthItems[0]!== undefined && this.monthItems[0].state === YearMonthPlanState.Passed ?
                  vIf(
                    <a-button type="danger" onClick={() => this.backPlans()}>
                      <a-icon type="rollback" />
                      退回
                    </a-button>,
                    vP(this.permissions,permissionsSmCrPlan.MonthOfYearPlan.Back),
                  ):undefined
              }
              {this.totalCount > 0
                ?
                ...
                return [
                  <span>
                    {vIf(
                      ...
                      vP(this.permissions, permissionsSmCrPlan.MonthOfYearPlan.Update),
                    )}
                  </span>,
                ];
              },
        ...
        />
        <SmCrPlanMonthOfYearPlanModal
          ref="SmCrPlanMonthOfYearPlanModal"
          axios={this.axios}
