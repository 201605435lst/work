...
export default {
  ...
  props: {
    ...
    workOrderIds: { type: Array, default: null },
  },
  data() {
    return {
      iVisible: false,
  ...
  watch: {
    workOrderIds: {
      handler: function (value) {
        this.iWorkOrderIds = value.map(item => { return item; });
        this.refresh();
      },
      immediate: true,
    },
  ...
  methods: {
    ...
    async refresh(resetPage = true) {
      ...
      if (resetPage) {
        this.pageIndex = 1;
        this.queryParams.maxResultCount = 5;//paginationConfig.defaultPageSize;
      ...
      this.queryParams.startTime = moment(this.time).date(1)
        .hours(0)
        .minutes(0)
        .seconds(0)
        .format('YYYY-MM-DD HH:mm:ss');

      this.queryParams.endTime = moment(this.time).endOf('month')
        .format('YYYY-MM-DD HH:mm:ss');

      let response = await apiMaintenanceRecord.getMaintenanceRecord(this.queryParams, this.repairTagKey, this.workOrderIds);
                ...
                >
                  <div slot="description">
                    ...
                    <div>
                      ...
                      {item.checkUserName != "" ? [<span>|&nbsp;&nbsp;验收人：{item.checkUserName}</span>] : undefined}
                    </div>
                  </div>
                </a-list-item-meta>
              </a-list-item>
      ...
      <div class="sm-cr-plan-maintenance-record">
        ...
        <a-descriptions bordered column={2}>
          ...
          {/* <a-descriptions-item label="检修时间" span={2}>
            <span>
              <a-month-picker value={this.time} onChange={value => this.time = value} />
              {/* 
              //disabled
              <a-range-picker
                placeholder={['开始月', '结束月']}
