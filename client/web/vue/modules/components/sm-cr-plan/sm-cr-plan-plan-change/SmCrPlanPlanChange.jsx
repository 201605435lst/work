...
export default {
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          dataIndex: 'newDate',
          width: 160,
          scopedSlots: { customRender: 'newDate' },
        },
        {
          title: '计划数量',
          dataIndex: 'count',
  ...
  methods: {
              ...
              for (let j = 0; j < dailyPlanResponse.data.length; j++) {
                ...
                if (item.id == tempItem.id) {
                  ...
                  if (this.pageState === PageState.View) {
                    ...
                    console.log(item);
                    if (data.state == YearMonthPlanState.UnCommit) {
    ...
    selectCangedPlans(selectVal) {
      ...
      let selectedPlans =
        _selectVal.length > 0
          ? _selectVal.filter(item => this.selectedChangePlanIds.indexOf(item.id) === -1)
          : [];

      for (let i = 0; i < selectedPlans.length; i++) {
        let item = selectedPlans[i];
        ...
        this.selectedChangePlans.push(item);
        this.selectedChangePlanIds.push(item.id);
      }
    },
    ...
    async commit() {
      this.form.validateFields(async (err, values) => {
        if (!err) {
          ...
          if (values.planTime === values.alterTime) {
            this.$message.warning('申请变更时间不能和原计划日期相同');
            return;
          }
          ...
          for (let item of this.selectedChangePlans) {
            let temp = {
              ...
              alterDateTime: item.alterDateTime,
            };
          ...
          if (this.pageState === PageState.Add) {
            ...
            if (requestIsSuccess(response)) {
              ...
              response = await apiAlterRecord.submitForExam({
                id: ent.id,
                repairTagKey: this.repairTagKey,
  ...
  render() {
    ...
    for (let item in SelectablePlanType) {
      if (SelectablePlanType[item] != 4 && this.repairTagKey == RepairTagKeys.RailwayWired) {
      ...
      if (SelectablePlanType[item] === 1 && this.repairTagKey == RepairTagKeys.RailwayHighSpeed) {
        alterTypeOptions.push(
          <a-select-option key={SelectablePlanType[item]}>
            {getSelectablePlanType(SelectablePlanType[item])}表
          </a-select-option>,
        ...
        <a-form form={this.form}>
          ...
          <a-row gutter={24}>
            <a-col sm={this.getChangeColsWidth[0]} md={this.getChangeColsWidth[0]}>
              <a-form-item
                label="原计划日期"
                label-col={{ span: this.getChangeColsWidth[1] }}
                wrapper-col={{ span: this.getChangeColsWidth[2] }}
              >
                <a-month-picker
                  allowClear={false}
                  style="width: 100%"
            ...
            <a-col sm={this.getChangeColsWidth[0]} md={this.getChangeColsWidth[0]}>
              <a-form-item
                label="申请变更时间"
                label-col={{ span: this.getChangeColsWidth[1] + 1 }}
                wrapper-col={{ span: this.getChangeColsWidth[2] - 1 }}
              >
                <a-month-picker
                  allowClear={false}
                  style="width: 100%"
                ...
                <a-table
                  ...
                  {...{
                    scopedSlots: {
                      ...
                      newDate: (text, record) => {
                        let newDateTime = moment(record.planDate).set('month', moment(this.newDate).get("month"));
                        if (this.pageState == PageState.Add) {
                          record.alterDateTime = newDateTime;
                        }
                        return [
                          <a-date-picker
                            disabled={this.pageState === PageState.View}
                            ...
                            // value={record.alterDateTime === undefined ? newDateTime : record.alterDateTime}
                            defaultValue={record.alterDateTime === undefined ? newDateTime : moment(record.alterDateTime).format('YYYY-MM-DD')}
                            onChange={(date, dateString) => {
                              record.alterDateTime = moment(date).format('YYYY-MM-DD');
                            }}
                          >
                          </a-date-picker>,
                        ];
