...
import { requestIsSuccess, getWorkContentTypeTitle } from '../../_utils/utils';
...
const formFields = ['workAreaId', 'workSiteIds'];
export default {
  ...
  props: {
    ...
    workAreaId: { type: String, default: '' },//作业工区
    isChange:{tyoe:Boolean,default:false},
  },
  data() {
    return {
      iId: null,
      ...
      if (requestIsSuccess(response)) {
        ...
        planDetails = planDetails.map(item => {
          return {
            ...
            planDate: item.dailyPlan ? moment(item.dailyPlan.planDate).format('YYYY-MM-DD') : null,
            // unFinishCount: null,
        ...
        this.workContent =
          _otherPlan.workContentType == WorkContentType.OtherPlan ? _otherPlan.workContent : null;

        this.workContentType = _otherPlan.workContentType;
    ...
    save() {
      this.form.validateFields(async (err, values) => {
        if (!err) {
          ...
          data.workContent =
            this.workContentType == WorkContentType.OtherPlan ? this.workContent : null;
          data.planDetails =
            ...
            <a-col sm={24} md={24}>
              <a-form-item label="计划内容" label-col={{ span: 2 }} wrapper-col={{ span: 22 }}>
                {this.workContentType == WorkContentType.MonthYearPlan ? (
                  ...
                  <a-input
                    type="textarea"
                    value={this.workContent}
                    disabled={this.pageState == PageState.View}
                    onChange={val => {
                      this.workContent = val.target.value;
                    }}
                  ></a-input>
                )}
              </a-form-item>
