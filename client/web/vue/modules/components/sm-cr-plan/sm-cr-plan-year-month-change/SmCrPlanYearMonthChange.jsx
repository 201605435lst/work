...
import { RepairTagKeys, YearMonthPlanType, YearMonthPlanState } from '../../_utils/enum';
...
export default {
  ...
  props: {
    ...
    alterRecordId: { type: String, default: null },
    isCreateRecord:{type:Boolean,default:false},
  },
  data() {
    return {
      file: {},
      ...
      queryParams: {
        ...
        remark: null, //变更原因
        alterRecordId: this.alterRecordId,
        isCreateRecord:this.isCreateRecord,
      },
    ...
    planType: {
      handler: function(value, oldValue) {
        if (value) {
          ...
          this.queryParams.planType = this.planType;
          this.initAxios();
          this.refresh();
        }
      },
  ...
  methods: {
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise(async (resolve, reject) => {
            ...
            let response = await apiYearMonthPlan.submitChangeForExam(
              _this.queryParams,
              _this.repairTagKey,
            );
            if (requestIsSuccess(response)) {
    ...
    async refresh(resetPage = true) {
      this.isLoading = true;
      this.queryParams.alterRecordId = this.alterRecordId;
