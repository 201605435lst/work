...
import SmCrPlanSkylightPlanWorkTicketModal from '../sm-cr-plan-skylight-plans/SmCrPlanSkylightPlanWorkTicketModal';
import { getRepairLevelTitle } from '../../_utils/utils';
...
export default {
  name: 'SmCrPlanMaintenanceWorkCheck',
  props: {
  ...
  methods: {
    ...
    async view(record) {
      if (record && record.secondARKey != null) {
        this.$refs.WorkflowModal.isInitial = false;
        this.$refs.WorkflowModal.edit(record.secondARKey);
      } else {
    ...
    async refresh(resetPage = true, page) {
      ...
      console.log(data);
      let response = await apiMaintenanceWork.getListForSecondStep({
    ...
    addWorkTick(record) {
      ...
      this.$refs.SmCrPlanSkylightPlanWorkTicketModal.add(record);
    },
  },
  render() {
    //状态选择
    ...
    return (
      <div class="sm-cr-plan-maintenance-work">
          ...
          {...{
            scopedSlots: {
              ...
              repairLevel: (text, record, index) => {
                let result = getRepairLevelTitle(record.repairLevel);
                return (
                  <a-tooltip placement="topLeft" title={result}>
                    <span>{result}</span>
                  </a-tooltip>
              ...
              workState: (text, record, index) => {
                if (record.secondARKey != null) {
                  let result = getWorkflowState(record.secondWorkflowState);
                  return (
                    <a-tooltip placement="topLeft" title={result}>
                      <span>{result}</span>
                    </a-tooltip>
                  ...
                  return (
                    <a-tooltip placement="topLeft" title='待复验'>
                      <span>待复验</span>
                    </a-tooltip>
                  );
                }
              ...
              operations: (text, record) => {
                return <span>
                  ...
                  {
                    record.secondWorkflowState == WorkflowState.Finished ||
                      record.secondWorkflowState == WorkflowState.Waiting ? null :
                      [<a-divider type="vertical" />,
                        <a
                          onClick={() => {
                            this.addWorkTick(record);
                          }}
                        >
                          提交审批
                        </a>]
        ...
        />
        <SmCrPlanSkylightPlanWorkTicketModal
          ref="SmCrPlanSkylightPlanWorkTicketModal"
          axios={this.axios}
