...
export default {
  name: 'SmCrPlanMonthPlanModal',
  props: {
    value: { type: Boolean, default: null },
    axios: { type: Function, default: null },
    repairTagKey: { type: String, default: null }, //维修项标签
    state: { type: Number, default: null }, //月表状态
  },
  data() {
    return {
  ...
  computed: {
    ...
    indeterminate() {
      return (
        this.record.skyligetType &&
        this.skyligetTypes.length > this.record.skyligetType.length &&
        this.record.skyligetType.length > 0
      );
    },
    isAllCheck() {
      return (
        this.record.skyligetType && this.skyligetTypes.length == this.record.skyligetType.length
      );
    },
  },
  created() {
    ...
    async inintData() {
      ...
      if (requestIsSuccess(response)) {
        ...
        if (this.repairTagKey == RepairTagKeys.RailwayHighSpeed) {
          ...
          if (index != -1) this.skyligetTypes.splice(index, 1);
        }
      }
    },
    //编辑按钮
        ...
        this.form.validateFields(async (err, values) => {
          if (!err) {
            ...
            if (sum > 0) {
              if (this.repairTagKey === RepairTagKeys.RailwayWired) {
                if (
                  _values.compiledOrganization == null ||
                  _values.compiledOrganization == undefined ||
                  _values.compiledOrganization.trim().length === 0
                ) {
              ...
              if (
                _values.equipmentLocation == null ||
                _values.equipmentLocation == undefined ||
                _values.equipmentLocation.trim().length === 0
              ) {
            ...
            if (
              !isNaN(Number(this.record.times)) &&
              sum !== _values.total * parseFloat(this.record.times)
            ) {
              this.$message.warning('当前月计划数量之和不等于总数量*每月次数，请重新维护数据');
              return;
            }
            let data = {
              ..._values,
              ...
              compiledOrganization: _values.compiledOrganization
                ? _values.compiledOrganization.trim()
                : '',
            };
  ...
  render() {
            ...
            <a-col sm={24} md={4}>
              ...
              >
                <a-input-number
                  ...
                  style="width:100%"
                  disabled={
                    this.state === YearMonthPlanState.Checking ||
                    this.state === YearMonthPlanState.Passed
                  }
                  min={0}
                  precision={3}
                  v-decorator={[
    ...
    return (
      <a-modal
        class="sm-cr-plan-month-plan-modal"
        width={1000}
        title={`${this.title}月计划`}
              ...
              <a-form-item label="总数量" label-col={{ span: 6 }} wrapper-col={{ span: 18 }}>
                <a-input
                  ...
                  {...{
                    scopedSlots: {
                      addonAfter: () => {
                        let value = !isNaN(Number(this.record.times))
                          ? `每月${this.record.times}次`
                          : this.record.times;
                        return (
                          <a-tooltip title={value} placement="topLeft">
                            <div class="addonAfter">{value}</div>
                          </a-tooltip>
                        );
                      },
                    },
