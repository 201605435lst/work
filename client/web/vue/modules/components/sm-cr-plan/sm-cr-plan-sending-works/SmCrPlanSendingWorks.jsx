...
export default {
  ...
  data() {
    return {
      ...
      pageIndex: 1,
      queryParams: {
        startPlanTime: moment(moment()).startOf('month').format("YYYY-MM-DD"), // 计划开始日期(默认本月1号)
        endPlanTime: moment(moment()).endOf('month').format("YYYY-MM-DD"), //计划结束日期（默认本月最后一天）
      ...
      rangePicker: {
        format: 'YYYY-MM-DD',
        value: [moment(moment()).startOf('month'), moment(moment()).endOf('month')],
      }, // 时间选择控件
  ...
  methods: {
    ...
    // 详细
    view(record) {
      console.log(record.orderState);
      this.$emit('view', SendWorkOperatorType.View, record.id, record.orderState);
    },
    // 派工单
    dispatch(record) {
      this.$refs.SmCrPlanSendingWorkModal.dispatch(record.id, record.orderState);
    },
    // 完成
    finish(record) {
      if (
        record.orderState === OrderState.NaturalDisasterCancel ||
        record.orderState === OrderState.OrderCancel ||
        record.orderState === OrderState.OtherReasonCancel
      ) {
        this.$message.error('该派工已经取消');
        return;
      }
      ...
      this.$emit('finish', SendWorkOperatorType.Finish, record.id, record.orderState);
    },
    // 验收
    acceptance(record) {
      if (record.workLeader === null) {
      ...
      this.$emit('acceptance', SendWorkOperatorType.Acceptance, record.id);
    },
    // 撤销
    remove(record) {
      if (
  ...
  render() {
    return (
      <div class="sm-cr-plan-vertical-skylight-plans">
        {/* 操作区 */}
        <sc-table-operator onSearch={this.refresh} onReset={this.reset} advancedCount={20}>
            ...
            scopedSlots: {
              ...
              level: (text, record) => {
                ...
                return this.repairTagKey === RepairTagKeys.RailwayHighSpeed &&
                  record.planType === PlanType.OutOf ? (
                    ''
                  ) : (
              ...
              orderState: (text, record) => {
                return (
                  <a-tag color={this.getTagColor(record.orderState)}>
                ...
                return [
                  <span>
                    ...
                    {vIf(
                      ...
                      vP(this.permissions, permissionsSmCrPlan.SendingWork.Detail) &&
                      vP(this.permissions, [
                        permissionsSmCrPlan.SendingWork.DispatchList,
                        permissionsSmCrPlan.SendingWork.Finish,
                        permissionsSmCrPlan.SendingWork.Acceptance,
                        permissionsSmCrPlan.SendingWork.Revoke,
                      ]),
                    )}
