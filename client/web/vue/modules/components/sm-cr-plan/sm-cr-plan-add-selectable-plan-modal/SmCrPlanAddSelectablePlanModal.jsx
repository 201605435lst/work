...
import { RepairTagKeys, SelectablePlanType, SkylightType } from '../../_utils/enum';
...
export default {
  ...
  methods: {
    ...
    getDefaultDateRange() {
      ...
      let range = [
        this.repairTagKey === RepairTagKeys.RailwayHighSpeed
          ? moment(year + '/' + month + '/' + date, this.dateFormat)
    ...
    async allSelect() {
      ...
      this.queryParams.maxResultCount = 1000;
      await this.refresh(false);
      this.selectedPlans = this.plans.map(x => x.id);
    ...
    return (
      ...
      >
        ...
        >
          ...
          <a-form-item label="是否为变更计划">
            <a-select
              value={this.queryParams.isChange}
              onChange={value => {
                this.queryParams.isChange = value;
                this.refresh();
                console.log(value);
              }}>
          ...
          <template slot="buttons">
            <a-button
              ...
              type="primary"
              disabled={this.plans.length === 0}
              loading={this.allSelectLoading}
            >
              {`${this.isAllSelect ? '全选' : '取消全选'}`}
            </a-button>
          </template>
        </sc-table-operator>
        ...
        <a-table
          ...
          {...{
            scopedSlots: {
              ...
              isChange:(text, record) => {
                return record.isChange?"是":"否";
              },
            },
          }}
        ></a-table>
