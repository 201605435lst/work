...
export default class Api {
  ...
  // 获取全部待选计划
  async getSelectableAllPlans(params, repairTagKey) {
    return await this.axios({
      url: `${url}/getSelectableAllPlans`,
      method: 'get',
      params: { ...params, repairTagKey },
    });
  }
  ...
  // 根级id获取待选计划
  async getSelectablePlanByIds(data) {
    return await this.axios({
      url: `${url}/forSelectablePlanByIds`,
      method: 'post',
      data,
      // params: { ...params, repairTagKey },
  ...
  // 导出
  async export(id,signatureName) {
    let data = { id: id ,signatureName:signatureName};
    return await this.axios({
      url: `${url}/export`,
