<template>
  <a-layout :class="['layout', device]">
    ...
    >
      ...
      <!-- layout content -->
      <a-layout-content
        :style="{ height: '100%', margin: '0', paddingTop: fixedHeader ? '64px' : '0' }"
      >
...
export default {
  ...
  computed: {
    ...mapState({
      // 动态主路由
      mainMenu: (state) => state.router.addRouters,
    }),
    contentPaddingLeft() {
  ...
  created() {
    ...
    this.collapsed = !this.sidebarVisible;
  },
  mounted() {
  ...
  methods: {
    ...mapActions(storeApp, [atApp.SetSidebar]),
    toggle() {
      console.log(atApp);
      this.collapsed = !this.collapsed;
      this[atApp.SetSidebar](!this.collapsed);
