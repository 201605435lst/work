<template>
  <div class="main">
    <a-form id="formLogin" ref="formLogin" class="user-layout-login" :form="form" @submit="handleSubmit">
      ...
      <!-- <a-form-item>
        <a-checkbox v-decorator="['rememberMe', { valuePropName: 'checked' }]" style="color:#eee">
          自动登录
        </a-checkbox>
        <router-link
...
import { timeFix, requestIsSuccess } from '@/utils/util';
...
export default {
  ...
  created() {
    // get2step({})
    //   .then(res => {
    //     this.requiredTwoStepCaptcha = res.data.result.stepCode
    //   })
    //   .catch(() => {
  ...
  methods: {
    ...mapActions(storeApp, [atApp.Login, atApp.Logout]),
    // handler
    ...
    handleSubmit(e) {
      ...
      validateFields(validateFieldsKey, { force: true }, async (err, values) => {
        if (!err) {
          ...
          params[!state.loginType ? 'email' : 'username'] = values.username;
          params.password = values.password;

          // getOpenIdConfig
          const resOpenIdConfig = await apiApp.getOpenIdConfig();
          ...
          // getJwks
          const resJwks = await apiApp.getJwks(resOpenIdConfig.data.jwks_uri);
          ...
          // 登录
          const resToken = await this[atApp.Login](params);
    ...
    //             message: '提示',
    //             description: '验证码获取成功，您的验证码为：' + res.data.result.captcha,
    //             duration: 8
    //           })
    //         })
    //         .catch(err => {
