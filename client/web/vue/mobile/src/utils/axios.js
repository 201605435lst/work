...
instance.interceptors.request.use(
  config => {
    const tokenValue = Vue.ls.get(config.tokenKey);
    if (tokenValue) {
      config.headers.common["Authorization"] = tokenValue;
      config.headers.common[config.storageOptions.organizationId] =
        localStorage.getItem(config.storageOptions.organizationId) || "";
    }
    return config;
  },
  error => {
...
instance.interceptors.response.use(
  ...
  error => {
    if (error.response) {
      const token = Vue.ls.get(config.tokenKey);
