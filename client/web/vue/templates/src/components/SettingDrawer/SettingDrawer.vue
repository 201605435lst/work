<template>
  <div ref="settingDrawer" class="setting-drawer">
    <a-drawer width="300" placement="right" :closable="false" :visible="visible" @close="onClose">
        ...
        <div :style="{ marginBottom: '24px' }">
          ...
          <a-alert type="warning" :style="{ marginTop: '24px' }">
            <span slot="message">
              ...
              <a
                href="https://github.com/sendya/ant-design-pro-vue/blob/master/src/config/default.js"
                target="_blank"
              >src/config/default.js</a>
            </span>
          </a-alert>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      visible: state => state.app.themeDrawerVisible,
    }),
  },
  watch: {},
  mounted() {
  ...
  methods: {
    ...mapMutations(storeApp, [mtApp.ToggleThemeDrawerVisible]),
    showDrawer() {
      // this.visible = true;
    ...
    onClose() {
      // this.visible = false;
      this.$store.commit(storeApp + '/' + mtApp.ToggleThemeDrawerVisible, false);
    },
    toggle() {
      // this.visible = !this.visible;
      this.$store.commit(storeApp + '/' + mtApp.ToggleThemeDrawerVisible, !this.visible);
    },
    onColorWeak(checked) {
      this.$store.dispatch(storeApp + '/' + atApp.ToggleWeak, checked);
      updateColorWeak(checked);
    },
    onMultiTab(checked) {
      this.$store.dispatch(storeApp + '/' + atApp.ToggleMultiTab, checked);
    },
    handleMenuTheme(theme) {
      this.$store.dispatch(storeApp + '/' + atApp.ToggleTheme, theme);
    },
    doCopy() {
  ...
  storageOptions: {
    ...
    handleLayout(mode) {
      this.$store.dispatch(storeApp + '/' + atApp.ToggleLayoutMode, mode);
      // 因为顶部菜单不能固定左侧菜单栏，所以强制关闭
      this.handleFixSiderbar(false);
    },
    handleContentWidthChange(type) {
      this.$store.dispatch(storeApp + '/' + atApp.ToggleContentWidth, type);
    },
    changeColor(color) {
      if (this.primaryColor !== color) {
        this.$store.dispatch(storeApp + '/' + atApp.ToggleColor, color);
        updateTheme(color);
      }
    },
    handleFixedHeader(fixed) {
      this.$store.dispatch(storeApp + '/' + atApp.ToggleFixedHeader, fixed);
    },
    handleFixedHeaderHidden(autoHidden) {
      this.$store.dispatch(storeApp + '/' + atApp.ToggleFixedHeaderHidden, autoHidden);
    },
    handleFixSiderbar(fixed) {
      if (this.layoutMode === 'topmenu') {
        this.$store.dispatch(storeApp + '/' + atApp.ToggleFixSiderbar, false);
        return;
      }
      this.$store.dispatch(storeApp + '/' + atApp.ToggleFixSiderbar, fixed);
    },
  },
};
</script>
