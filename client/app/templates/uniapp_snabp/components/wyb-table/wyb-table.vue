<template>
	<view class="wyb-table-box">
		...
		<view
			v-if="!loading"
			class="wyb-table-scroll-view"
			:style="{
				width: width,
				height: contents.length < 6 ? height : '460rpx', //更改了,原始：height
				borderTop: null, //更改了,原始：borderTop: '1px solid' + borderColor
				borderLeft: showLeftAndRightBorder ? '1px solid' + borderColor : 'none',
				borderRight: showLeftAndRightBorder ? '1px solid' + borderColor : 'none',
				backgroundColor: contentsSort.length == 0 ? '#ffffff' : '',
			}"
		>
			<view
				class="wyb-table-header"
				:style="{
					borderBottom: '1px solid' + borderColor,
					borderTop: '1px solid' + borderColor, //更改了,原始：borderBottom: '1px solid' + borderColor
				}"
			>
				<view
					class="wyb-table-header-item"
					v-if="enableCheck"
					:style="{
				...
				<view
					...
					:style="{
						...
						minWidth:
							(item.width ||
								(headers.length == 4 ? 188 : headers.length == 3 ? 250 : headers.length == 2 ? 375 : false) ||
								defaultColWidth) + 'rpx',
						maxWidth:
							(item.width ||
								(headers.length == 4 ? 188 : headers.length == 3 ? 250 : headers.length == 2 ? 375 : false) ||
						...
						padding: padding[0] + 'rpx ' + (padding[1] || padding[0]) + 'rpx',
						backgroundColor: headerBgColor,
						borderRight: index === headers.length - 1 || !showVertBorder ? 'none' : '1px solid' + borderColor,
						//更改了,原始：index === headers.length - 1 || (!showVertBorder && index !== 0) ? 'none' : '1px solid' + borderColor,
						zIndex: index === 0 ? 20 : 0,
				...
				>
					...
					<view
						...
						:style="{
							...
							borderRight: hIndex === headers.length - 1 || !showVertBorder ? 'none' : '1px solid' + borderColor,
							//更改了,原始：hIndex === headers.length - 1 || (!showVertBorder && hIndex !== 0) ? 'none' : '1px solid' + borderColor,
							zIndex: hIndex === 0 ? 20 : 0,
							left: enableCheck ? checkColWidth + 'rpx' : 0,
							position: hIndex === 0 && firstLineFixed ? 'sticky' : 'static',
						}"
					>
						<!-- 更改了,原始 v-else的 {{ autoContentItem(cIndex, hIndex) }}-->
						<!-- //删除图标 -->
						<view v-if="header.key === 'wyb-delete'" class="iconfont icon-ashbin" style="font-size: 44rpx; color: red" />
						<!-- //二维码扫描图标 -->
						<view v-else-if="header.key === 'wyb-QRcode'" style="display: flex; align-items: center">
							<image
								v-show="!(wybQRcodeConfirm.length > 0 && wybQRcodeConfirm.includes(cIndex))"
								:src="require('@/static/BIMAppUI/other/QRC.png')"
								style="width: 60rpx; height: 60rpx"
							/>
							<image
								v-show="wybQRcodeConfirm.length > 0 && wybQRcodeConfirm.includes(cIndex)"
								:src="require('@/static/BIMAppUI/other/confirm.png')"
								style="width: 50rpx; height: 50rpx"
							/>
						</view>
						<!-- //数字输入框 -->
						<uni-number-box
							v-else-if="header.key === 'wybNumberBox'"
							:disabled="disabled"
							:min="0"
							:max="99999999"
							:value="autoContentItem(cIndex, hIndex)"
							@change="
								val => {
									autoContentItem(cIndex, hIndex, 'wybNumberBox', val);
								}
							"
						></uni-number-box>
						<!-- //普通单元 -->
						<block v-else>{{ autoContentItem(cIndex, hIndex) }}</block>
					</view>
				</view>

				<!-- //更改了-添加空数据提示,原始：无<view v-if="contentsSort.length == 0">-->
				<view
					v-if="contentsSort.length == 0"
					style="
						display: flex;
						min-height: 150rpx;
						max-height: 150rpx;
					...
					"
				>
					<view class="iconfont icon-empty-a" style="font-size: 60rpx; padding-right: 10rpx" />
...
export default {
	...
	computed: {
		...
		autoContentItem() {
			return function (cIndex, hIndex, operation, val) {
				let content = this.contentsSort[cIndex];
				let header = this.headers[hIndex];
				let result = '';
				if (operation && operation == 'wybNumberBox') {
					content[header.key] = val;
				} else if (content[header.key] || content[header.key] === 0) {
