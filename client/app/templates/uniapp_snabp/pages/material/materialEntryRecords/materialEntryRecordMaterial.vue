...
<template>
	<view id="pages-uni-entry-record-material">
		<uni-forms ref="form" :value="material" :rules="rules">
			...
			<uni-forms-item label="入库数量" required name="count">
				<input
					class="uni-form-item-input"
					style="padding: 0 20rpx"
					type="number"
...
import * as apiSupplier from '@/api/material/supplier.js';
	...
	methods: {
		...
		submit() {
			...
			this.$refs.form
				...
				.then(async res => {
					if (res.count <= 0) {
						showToast('入库数量不能小于等于0', false);
					} else if (res.price <= 0) {
						showToast('价格不能小于等于0', false);
					} else {
						uni.$emit('addMaterial', {...res, materialId: _this.material.id, supplierId: res.supplier.id});
						uni.navigateBack();
					}
				})
				.catch(err => {
