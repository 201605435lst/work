	...
	<view class="pages-fileManage-file-fileTransfer">
		...
		<view class="file-top">
			<view class="construction-dispatchs-oprator-view">
				<view :class="waitApprove ? 'approve-selected' : ''" @tap="onTap(true)">下载列表</view>
				<view :class="waitApprove ? '' : 'approve-selected'" @tap="onTap(false)">上传列表</view>
			</view>
		</view>
		<!-- 加载中 -->
		...
		<view style="padding-top: 80rpx" v-else>
			<view class="fileTransfer-list-top">
				<text v-if="waitApprove">已下载：{{ dataSources.length }}</text>
				<text v-else>已上传：{{ dataSources.length }}</text>
			...
			<block v-for="(item, index) of dataSources" :key="index">
				<view class="file-item" @tap="clickFileFolder(item)">
					...
					<view @tap.stop="selectButton(item)">
						<image v-if="item.selected" :src="require('@/static/BIMAppUI/other/downloading.png')" />
						<image v-else :src="require('@/static/BIMAppUI/other/downloaded.png')" />
					</view>
				</view>
			</block>
