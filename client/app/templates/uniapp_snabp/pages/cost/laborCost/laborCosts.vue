<template>
	<view class="uni-labor-costs">
		<!-- 操作区 -->
		<view class="uni-labor-costs-oprator">
		...
		<view class="uni-list-empty" v-if="!(laborCosts && laborCosts.length > 0)">
			<view class="iconfont icon-empty-a" />
			<view class="uni-list-empty-title">暂无数据</view>
		</view>
		<!-- 展示区 -->
		<view style="padding-top: 80rpx" v-else>
			<view class="uni-labor-costs-item" v-for="item in laborCosts" :key="item.id" @click="onView(item.id)">
				<view class="item-content first-line">
					<view class="labor-cost-item-code">施工专业： {{ item.professionName }}</view>
				...
				<view class="omit-content">付款日期： {{ item.date }}</view>
				<view class="omit-content">收款单位：{{ item.payeeName }}</view>
				<view class="omit-content">费用金额：{{ item.money }}(万元)</view>
				<view class="omit-content">备注：{{ item.remark || '--' }}</view>
			</view>
			<uni-load-more color="#ffffff" :status="loadStatus"></uni-load-more>
		</view>

		<laborCostDrawer ref="TableOperator" @change="onChange" />
	</view>
</template>

<script>
import laborCostDrawer from './laborCostDrawer.vue';
...
export default {
	...
	methods: {
		...
		async refresh(reset = true, isClearData = false) {
			...
			const response = await apiLaborCost.getList({
			...
			if (requestIsSuccess(response) && response.data && response.data.items) {
				...
				let array = response.data.items.map(item => {
					return {
						...
						payeeName: item.payee ? item.payee.name : '',
						professionName: item.professional ? item.professional.name : '',
					};
				});
				this.laborCosts = this.laborCosts.concat(array);
			}
			this.loading = false;
		},
		...
		onAdd() {
			uni.navigateTo({
				url: `../../../pages/cost/laborCost/laborCost?pageState=${PageState.Add}`,
			});
		},
		...
		onEdit(id) {
			uni.navigateTo({
				url: `../../../pages/cost/laborCost/laborCost?pageState=${PageState.Edit}&id=${id}`,
			});
		},
		...
		onView(id) {
			uni.navigateTo({
				url: `../../../pages/cost/laborCost/laborCost?pageState=${PageState.View}&id=${id}`,
			});
		},
		...
		async remove(id) {
			if (id) {
				uni.showModal({
					...
					success: async res => {
						if (res.confirm) {
							let response = await apiLaborCost.remove([id]);
